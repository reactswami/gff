...
const dashboardSelect = el => el.checked === true && el?.uid?.indexOf('StatseekerDefault') === -1;
...
export class ManageDashboardsCtrl {
  ...
  refreshList() {
    ...
    return this.searchSrv
      ...
      .then(() => {
        ...
        return this.backendSrv.getFolderByUid(_this.folderUid).then(folder => {
  ...
  initDashboardList(result: any) {
    ...
    this.sections =
      result.sort((x, y) => {
        return x.hasOwnProperty('uid') && x.uid === 'StatseekerDefault'
          ? -1
          : y.hasOwnProperty('uid') && y.uid === 'StatseekerDefault' ? 1 : 0;
      }) || [];
  ...
  selectionChanged() {
    ...
    for (const section of this.sections) {
      selectedDashboards += section.items.filter(dashboardSelect).length;
    }

    const selectedFolders = this.sections.filter(dashboardSelect)
      .length;
  ...
  getFoldersAndDashboardsToDelete() {
    ...
    for (const section of this.sections) {
      if (section.checked && section.id !== 0 && section.uid.indexOf('StatseekerDefault') === -1) {
        selectedDashboards.folders.push(section.uid);
      } else {
        const selected = section.items.filter(dashboardSelect);
